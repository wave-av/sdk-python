"""Verifies every `wave.<namespace>.<method>(` call referenced in README.md's
...
CALL_RE = re.compile(r"\bwave\.(\w+)\.(\w+)\(")
...
def test_readme_quickstart_calls_are_real():
    ...
    for namespace, method in calls:
        ...
        if ns is None:
            missing.append(f"wave.{namespace} (namespace does not exist)")
        elif not hasattr(ns, method):
            missing.append(f"wave.{namespace}.{method} (method does not exist)")
